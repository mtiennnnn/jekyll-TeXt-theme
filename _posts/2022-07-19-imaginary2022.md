...
Ta có thể sử dụng `binding` để thay thế cho `child_process` như thường thấy vì `require` không thể dùng được, và nếu như mình không sai gì và dựa trên hình trên thì bản chất của `child_process` nó cũng dựa trên thằng `process.binding` này.
